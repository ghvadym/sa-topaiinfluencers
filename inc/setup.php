...
function action_custom_columns_content($column_id, $post_id)
{
    ...
    foreach ($socials as $socialKey) {
        if ($column_id !== $socialKey) {
            continue;
        }

        $value = get_post_meta($post_id, $socialKey, true);

        echo $value ? '<i class="subscribers_status status_true"></i>' : '<i class="subscribers_status status_false"></i>';
    }
}